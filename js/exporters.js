// File output helpers: binary STL, a minimal ZIP (stored, no compression), and download.

// Binary STL from a three.js BufferGeometry (indexed or not). Units: mm.
export function stlFromGeometry(geometry) {
...
export function slug(text, fallback = 'keychain') {
  const s = String(text).toLowerCase().replace(/[^a-z0-9]+/g, '-').replace(/^-+|-+$/g, '').slice(0, 30);
  return s || fallback;
}
